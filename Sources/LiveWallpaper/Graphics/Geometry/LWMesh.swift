...
public final class LWMesh {
  ...
  public var kernelShader: MTLFunction?
  public var mtkMesh: MTKMesh?
  public var material: LWMaterial?
  ...
  func render(encoder: MTLRenderCommandEncoder) {
    ...
    if let mesh = mtkMesh {
      encoder.setVertexBuffer(mesh.vertexBuffers[0].buffer, offset: 0, index: LWRenderer.firstFreeVertexBufferIndex)
...
//    // Execute kernel shader if available
//    if let kernelShader = kernelShader {
//      encoder.setComputePipelineState(Renderer.computePipelineState(for: kernelShader))
