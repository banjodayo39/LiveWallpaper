...
public final class LWNode {
  ...
  /**
   ...
   the node etc.
   */
  public var update: ((_ time: Time, _ node: LWNode) -> Void)?
  ...
  private var children = [LWNode]()

  public init(mesh: LWMesh? = nil) {
    self.mesh = mesh
  }

  /// Adds a new child to the nodes children collection at the end
  public func addChild(_ child: LWNode) {
    children.append(child)
  }

  /// Adds a new child in the nodes children collection at the specified index
  public func insertChild(_ child: LWNode, index: Int) {
    children.insert(child, at: index)
  }

  /// Removes a child from the node
  public func removeChild(_ child: LWNode) {
  ...
  func render(
    ...
    if let mesh = mesh, let material = mesh.material {
      ...
      if let texture0 = material.mainTexture {
        self.texture0 = texture0.mtlTexture
        encoder.setFragmentTexture(texture0.mtlTexture, index: 0)
        encoder.setFragmentSamplerState(texture0.samplerState, index: 0)
      }

      if let texture1 = material.secondaryTexure {
        self.texture0 = texture1.mtlTexture
